...
int main()
{
    ...
    std::cout << "unsigned int  l_data8  = 1u << 11: " << std::bitset<8>(l_data8) << std::endl;

    unsigned int  l_data9 = l_data8 << 21;
    std::cout << "unsigned int  l_data9 = l_data8 << 21: " << std::bitset<8>(l_data9) << std::endl;

    unsigned int  l_data10  = 0xFFFFFFFF >> 5;
    std::cout << "unsigned int  l_data10  = 0xFFFFFFFF >> 5: " << std::bitset<8>(l_data10) << std::endl;

    unsigned int  l_data11 = 0b1001 ^ 0b01111;
    std::cout << "unsigned int  l_data11 = 0b1001 ^ 0b01111: " << std::bitset<8>(l_data11) << std::endl;

    unsigned int  l_data12 = ~0b1001;
    std::cout << "unsigned int  l_data12 = ~0b1001: " << std::bitset<8>(l_data12) << std::endl;

    unsigned int  l_data13 = 0xF0 & 0b1010101;
    std::cout << "unsigned int  l_data13 = 0xF0 & 0b1010101: " << std::bitset<8>(l_data13) << std::endl;

    unsigned int  l_data14 = 0b001 | 0b101;
    std::cout << "unsigned int  l_data14 = 0b001 | 0b101: " << std::bitset<8>(l_data14) << std::endl;

    unsigned int  l_data15 = 7743;
    std::cout << "unsigned int  l_data15 = 7743: " << std::bitset<8>(l_data15) << std::endl;

    int  l_data16 = -7743;
    std::cout << "int  l_data16 = -7743: " << std::bitset<8>(l_data16) << std::endl;
    ...
    i_vec_reg_dst = 0x15;
    i_vec_reg_src_0 = 0x06;
    i_vec_reg_src_1 = 0x1B;
    result = 0b01010101010110110101010001110101;
    if (instruction_asimd_compute(i_vec_instr, i_vec_reg_dst, i_vec_reg_src_0, i_vec_reg_src_1) == result)
        std::cout << "Success!" << std::endl;
    else
        std::cout << "Failed!" << std::endl;
                                        ...
                                        unsigned char i_vec_reg_src_1 )
{
    ...
    return (i_vec_instr & 0xFE0FFC00) | (i_vec_reg_dst & 0xE0) //Bits 0-4 should be set to bits 0-4 of i_vec_reg_dst
                                      | (i_vec_reg_src_0 & 0xE0) << 5 //Bits 5-9 should be set to bits 0-4 of i_vec_reg_src_0
                                      | (i_vec_reg_src_1 & 0xE0) << 16; //Bits 16-20 should be set to bits 0-4 of i_vec_reg_src_1
}